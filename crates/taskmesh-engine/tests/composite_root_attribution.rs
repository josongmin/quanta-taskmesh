...
fn child_permit_rolls_into_root_accounting() {
    ...
    // Two children of the same root on distinct stages.
    let c1 =
        TaskSpec::blocking(TaskClass::new("worker")).child_of("root-1", TaskStage::new("parent"));
    let c2 = TaskSpec::cpu(TaskClass::new("worker")).child_of("root-1", TaskStage::new("parent"));
    ...
    assert_eq!(root.active_stages, 2); // "blocking" and "cpu"
}
...
fn child_saturation_bubbles_to_root_verdict() {
    ...
    // First child occupies the "blocking" target stage.
    let c1 =
        TaskSpec::blocking(TaskClass::new("worker")).child_of("root-2", TaskStage::new("parent"));
    // Second child targets a distinct stage ("cpu"), so it is not recursive —
    // it instead saturates the class cap, and that verdict bubbles up.
    let c2 = TaskSpec::cpu(TaskClass::new("worker")).child_of("root-2", TaskStage::new("parent"));
    assert!(matches!(
