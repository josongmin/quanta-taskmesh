...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn a_saturated_capability_pool_alone_is_enough_to_shed() {
    // The class quota here is generous (8) and only the *pool* is full, so this
    // isolates the capability authority: if capability occupancy were not part
    // of the admission decision, this submission would be admitted and run.
    let rt = Builder::new()
        .topology(TopologyConfig::new().blocking_threads(1))
        .resources(ResourceBudget::new().cpu_units(1000).memory_units(1000))
        .class_policy(
    ...
    let deadline = Instant::now() + Duration::from_secs(5);
    while rt.snapshot().capabilities["blocking"].in_use == 0 {
        assert!(Instant::now() < deadline, "holder never occupied the pool");
        tokio::time::sleep(Duration::from_millis(2)).await;
    }
    assert_eq!(
        rt.snapshot().classes[&TaskClass::new("c")].inflight,
        1,
        "the class quota is nowhere near its cap"
    );
    ...
    assert!(
        ...
        "a full capability pool must be rejected on its own, got {result:?}"
    );
    assert_eq!(
        ran.load(Ordering::SeqCst),
        0,
        "the shed job must not have executed"
    );

    release.send(()).expect("holder still waiting");
    holder.await.expect("join").expect("holder completes");
    assert_eq!(rt.snapshot().capabilities["blocking"].in_use, 0);
}
