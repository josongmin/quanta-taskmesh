...
- CP13 curated raw evidence showed `lease-return-does-not-wake-the-drain` broke both
  ...
  removed custody-return notification; the clean baseline passed all eight tests. The inventory
  correction declares that exact cofailure rather than allowing arbitrary failures. A focused
  rerun observed exactly those two failures with exit 101, selected/executed 1/1 and problems 0.
