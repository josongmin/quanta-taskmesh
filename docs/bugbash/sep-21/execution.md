...
- CP13 curated raw evidence showed `lease-return-does-not-wake-the-drain` broke both
  ...
  reproduced both, but CP14 hosted rerun observed only the primary oracle. Therefore the racing peer
  is not a stable exact cofailure. The root correction gives this entry an explicit representative
  test filter: baseline and mutant each run exactly one named test, single-threaded and `--exact`.
  Filters must equal the primary expected failure and match a Rust test-name grammar; controls,
  cofailures and harness-option injection are rejected.
