...
| F03 / B28 | REPRODUCED — fix awaits contract/API decision | `PermitId` and `Ticket` are `u64` aliases and every Governor starts at 1. `cross_governor_ids` proves collided foreign raw values cause local release, phase advance, claim, and abandon. Lease tokens have authority nonces; raw transition APIs do not. | `crates/taskmesh-engine/tests/cross_governor_ids.rs` (4/4 focused); `crates/taskmesh-engine/src/shared/mod.rs`, `crates/taskmesh-engine/src/engine/governor.rs` | BG25-001, BG25-004 |
| F04 / D21,D22,D25,H33 | PARTIAL — library boundary implemented; deployment adoption OPEN | The host now exposes bounded strict task/config parsers and Builder promotion with negative fixtures. Raw DTO deserialization remains permissive by compatibility design, and no in-repo production caller of the strict parsers was found. Whether external untrusted bytes use this boundary is an external ownership fact. | `crates/taskmesh/src/ingress.rs`, `crates/taskmesh/tests/strict_ingress.rs`; `crates/taskmesh-contract/src/task.rs` | BG25-001, BG25-002 |
