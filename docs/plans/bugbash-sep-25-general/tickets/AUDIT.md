...
| F04 / D21,D22,D25,H33 | PARTIAL — library boundary implemented; deployment adoption OPEN | The host exposes bounded strict task/config parsers and Builder promotion with negative fixtures. Raw DTO deserialization remains permissive by compatibility design, and no in-repo production caller of the strict parsers was found. Whether external untrusted bytes use this boundary is an external ownership fact. | `crates/taskmesh/src/ingress.rs`, `crates/taskmesh/tests/strict_ingress.rs`; `crates/taskmesh-contract/src/task.rs` | BG25-001, BG25-002 |
