...
| BG25-006 | `cargo test --locked -p taskmesh --test hardening_deadline_custody --test deadline_cancel --test hardening_drain --test runtime_cpu_executor` | `cargo test --locked -p taskmesh --test host_open_loop` |
...
| BG25-011 | `cargo test --locked -p taskmesh-bench --test hellgate --test inferno --test fairness_property` | `cargo test --locked -p taskmesh --test host_open_loop`; timing qualification remains separate |
