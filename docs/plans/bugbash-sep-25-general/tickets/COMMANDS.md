...
| BG25-002 | `cargo test --locked -p taskmesh-contract --test task_plan_validation --test contract_roundtrip` | `cargo test --locked -p taskmesh --test strict_ingress --test config_inventory`; rerun `--test strict_ingress` with `--features rayon` |
