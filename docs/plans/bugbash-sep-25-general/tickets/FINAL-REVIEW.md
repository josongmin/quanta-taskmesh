...
The 104-row static mapping validates on the current tree; this is structural evidence, not an execution receipt. New compound fixtures provide candidate evidence for mixed-substrate overload, independent capacity saturation, staged blocker release, policy interactions, concurrent histories, split child lifetimes, and host/simulator accounting. The in-repository semantic audit retains H04's combined race and H16's failure-schedule replay as explicit residuals. The macOS CI profile passed at committed, clean HEAD `3ac3ac46df854da785140ab37a0187363479fb12`; its receipt is valid only for that source and must be reissued after any tracked edit. D1–D9 human review and external ingress/planner/wire adoption remain OPEN under their external authorities; the Semantica concurrent consumer test now passes after a consumer-owned identity and error-projection fix, but only on a changing checkout.
...
| External consumer | Qualify the repaired Semantica concurrent path against the final Taskmesh source and resolve the separate lib-test compilation gap. | A focused QBC 2/2 on a changing checkout does not prove clean deployed adoption; see `EXTERNAL-ADOPTION.md`. |
