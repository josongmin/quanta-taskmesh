...
The final-audit delta at base `8a4f3a6` repairs existing-output corruption and
early-error cleanup, removes duplicate replay copying, and separates retained
runner bytes from the locally built replay validator. Its owner-local Rust 120 /
Python B07 36 cases and new H5 smoke passed. Plain historical replay is
structural consistency only; current-source replay also requires runner byte
equality. Clean CI and optimized oracle remain open.
