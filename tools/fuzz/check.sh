...
# than by the next nightly campaign. Clippy already performs Cargo's check
# compilation, so a preceding `cargo check` would build the same targets twice.
# `--all-targets` makes every `[[bin]]` target part of the check; `-D warnings`
# because a warning in a harness is usually a dropped `#[must_use]` outcome.
set -euo pipefail
cd "$(dirname "$0")/../../fuzz"
