#!/usr/bin/env python3
"""Verify the Rayon matrix selection and executed case denominator."""
...
def main() -> int:
    try:
        if sys.argv[1:] != ["--rayon"]:
            raise ValueError("the matrix gate requires --rayon")
        ...
        print(f"taskmesh-test-rayon status=FAIL reason={exc}", file=sys.stderr)
        return 1
